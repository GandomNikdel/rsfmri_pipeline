#!/usr/bin/env python3
"""
Stage 2B: DVARS QC v1.1.0 - boxplot-only decision version
...
Decision rule:
  DVARS review flag is based only on subject-specific boxplot outlier percentage:
    DVARS boxplot threshold = P75 + 1.5 * IQR
    HIGH_DVARS_REVIEW_BOXPLOT if >20% of volumes exceed this threshold.

Important:
  This stage does NOT make final exclusion decisions by itself.
  Final exclusion decisions should be made by combining:
...
PIPELINE_VERSION = "1.1.0-boxplot-only"
...
def run_command(cmd: List[str], log_path: Optional[Path] = None) -> Tuple[int, str]:
    proc = subprocess.run(
        cmd,
...
def fslnvols(nifti_path: Path, log_path: Path) -> Optional[int]:
...
def process_subject(sub_dir: Path) -> Dict[str, Any]:
    ...
    boxplot_mask_path = out_dir / "dvars_outlier_mask_boxplot.txt"
    boxplot_spikes_path = out_dir / "dvars_spike_regressors_boxplot.tsv"
    ...
    boxplot_threshold = q75 + BOXPLOT_IQR_MULTIPLIER * iqr
    boxplot_mask = [1 if v > boxplot_threshold else 0 for v in aligned_vals]

    boxplot_count = sum(boxplot_mask)
    boxplot_pct = 100.0 * boxplot_count / nvol

    flag_boxplot_pct = int(boxplot_pct > DVARS_OUTLIER_PERCENT_REVIEW)
    ...
    row.update({
        ...
        "dvars_boxplot_threshold": boxplot_threshold,
        "dvars_boxplot_outlier_count": boxplot_count,
        "dvars_boxplot_outlier_pct": boxplot_pct,
        "flag_dvars_boxplot_pct_gt_20": flag_boxplot_pct,
    })

    if flag_boxplot_pct:
        row["qc_status_preliminary"] = "HIGH_DVARS_REVIEW_BOXPLOT"
    else:
        row["qc_status_preliminary"] = "PASS_PRELIMINARY_DVARS_BOXPLOT"

    row["notes"] = (
        "Preliminary DVARS QC only. Decision is based on subject-specific "
        "boxplot outlier percentage. Final exclusion requires combined "
        "FD + DVARS + usable-data decision."
    )

    write_vector(boxplot_mask_path, boxplot_mask)
    write_timeseries_tsv(timeseries_tsv, aligned_vals, boxplot_mask)
    write_spike_regressors(boxplot_spikes_path, boxplot_mask, "dvars_boxplot_spike")
    ...
    metadata = {
        ...
        "preliminary_review_rule": (
            f"HIGH_DVARS_REVIEW_BOXPLOT if boxplot outlier percent > "
            f"{DVARS_OUTLIER_PERCENT_REVIEW}%"
        ),
        "important_note": (
            "This stage creates preliminary DVARS review flags only, "
            "not final exclusions."
        ),
    }
...
def write_dataset_outputs(rows: List[Dict[str, Any]]) -> None:
    ...
    summary_file = LOG_DIR / f"dvars_qc_v1_dataset_summary_boxplot_only_{RUN_STAMP}.tsv"
    review_file = LOG_DIR / f"dvars_qc_v1_review_candidates_boxplot_only_{RUN_STAMP}.tsv"
    config_file = LOG_DIR / f"dvars_qc_v1_config_boxplot_only_{RUN_STAMP}.json"
    master_log = LOG_DIR / f"dvars_qc_v1_master_boxplot_only_{RUN_STAMP}.log"
    stage_summary = LOG_DIR / f"dvars_qc_v1_stage_summary_boxplot_only_{RUN_STAMP}.txt"
    ...
    review_rows = [
        r for r in rows
        if r.get("qc_status_preliminary") != "PASS_PRELIMINARY_DVARS_BOXPLOT"
    ]
    ...
    config = {
        ...
        "important_note": (
            "This is preliminary DVARS QC using a boxplot-only review decision. "
            "It is not final exclusion."
        ),
    }
    ...
    boxplot_pct_values = numeric_list("dvars_boxplot_outlier_pct")
    ...
    with open(stage_summary, "w", encoding="utf-8") as f:
        ...
        write_dist(f, "DVARS boxplot outlier percentage distribution", boxplot_pct_values)

        f.write("Top 30 subjects by DVARS boxplot outlier percentage:\n")
        f.write("subject,qc_status,dvars_mean,dvars_max,dvars_boxplot_outlier_pct\n")

        for r in top_boxplot:
            f.write(",".join([
...
def main() -> int:
    ...
    if missing_cmds:
        print(
            "ERROR: Missing required FSL commands: " + ", ".join(missing_cmds),
            file=sys.stderr,
        )
    ...
    for idx, sub_dir in enumerate(subjects, start=1):
        print(f"[{idx}/{len(subjects)}] START {sub_dir.name}")
        row = process_subject(sub_dir)
        rows.append(row)
        print(f"[{idx}/{len(subjects)}] DONE  {sub_dir.name}  {row.get('qc_status_preliminary')}")
